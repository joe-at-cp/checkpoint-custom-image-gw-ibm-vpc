...
variable "Resource_Group" {
  default     = "Default"
  description = "The resource group that will be used when provisioning the Check Point custom image."
}
...
locals {
  gw_image_url    = "cos://${var.VPC_Region}/checkpoint-${var.VPC_Region}/Check_Point_${var.CP_Version}_Cloudguard_Security_Gateway.qcow2"
}

resource "ibm_is_image" "cp_gw_custom_image" {
  href             = "${local.gw_image_url}"
  name             = var.gw_image_name
